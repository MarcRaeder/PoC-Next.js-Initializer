...
async function run(): Promise<void> {
  const conf = new Conf({ projectName: 'create-processcube-app' })
...
async function notifyUpdate(): Promise<void> {
  try {
    ...
    if (res?.latest) {
      const updateMessage =
        packageManager === 'yarn'
          ? 'yarn global add create-processcube-app'
          : packageManager === 'pnpm'
          ? 'pnpm add -g create-processcube-app'
          : 'npm i -g create-processcube-app'

      console.log(
        chalk.yellow.bold('A new version of `create-processcube-app` is available!') +
          '\n' +
